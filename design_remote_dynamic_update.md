...
```mermaid
graph TD
    User[管理員/控制台] -- 1. 更新定義 --> CC[配置中心/資料庫<br/>Config Center / DB]
    ...
    Server -- 5. 推送 Notification<br/>(list_changed) --> Client[MCP Client/Claude]
    Client -- 6. 重抓工具清單<br/>(tools/list) --> Server
```
...
*   目的是優化 LLM 對該工具的理解，或對接新的後端 API 版本。

### 階段二：熱載入 (Hot-Reloading)
Sidecar 偵測到配置變更，並在內存中完成「換裝」：
*   **觸發機制**：可透過 Webhook、ConfigMap 掛載監聽、或是資料庫定期掃描。
*   **Spring AI 整合**：利用 Spring AI 的 `ToolCallback` 抽象，將舊的 Callback 實例替換為根據新配置生成的實例。

### 階段三：協議同步 (Protocol Sync)
這是 MCP 協議的精華，確保連線中的 AI 客戶端能獲得同步：
*   **行為**：MCP Server 透過 SSE (Server-Sent Events) 通道，向所有連線中的客戶端發送 `notifications/tools/list_changed`。
*   **效用**：客戶端收到此訊號後，會主動發起新的 `tools/list` 請求，更新其內部的工具工具箱 (Tooling Context)。

---

## 3. 技術優勢 (Key Benefits)

*   **⚡️ 零停機維護 (Zero-Downtime)**：調整 AI 提示詞 (Prompting) 或修正 API 參數時，不需要滾動更新 (Rolling Update) 整個服務。
*   **精準運維 (Precise Tweaking)**：當發現 AI 總是選錯工具時，可以即時修改描述並立即驗證效果。
*   **權限與版本控制**：可以針對不同的環境或使用者，動態配送不同的工具定義子集。

---

## 4. 未來實作建議 (Implementation Roadmap)

1.  **Spring Cloud Config / K8s ConfigMap**：作為首選的動態配置源。
2.  **McpServer Listener**：實作監聽器，當註冊表變更時自動調用 `mcpServer.notifyToolsListChanged()`。
3.  **Audit Logs**：記錄每一次工具定義的變更，以便追蹤對 AI 回應品質的影響。
